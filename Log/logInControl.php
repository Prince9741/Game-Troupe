        ...
        if($currentTimeInSec>=(int)strtotime($rows['AfterLogin'])){
            if($rows['Password']==$pwd){//check password is correct or not
                ...
                if($rows['LoginTry']){
                    ...
                    $sql="UPDATE `Players` SET `LoginTry` = $wrongPassTry, `AfterLogin`= '$currentTime' WHERE `userName`='$userName'";
                    $_SESSION['msg']='<div class="msg">Profile locked for '.$waiTimeInHour.' hour</div>';  
                }
                $Scoring->query($sql);
            }
        }
        else{
            $timeLeft=strtotime($rows['AfterLogin']);
            $timeLeft=$timeLeft-$currentTimeInSec;
            $timeLeft=(int)($timeLeft/60);
            $_SESSION['msg']='<div class="msg">Your id will unlocked after '.$timeLeft.'min </div>';                
        }
    }
    else{
